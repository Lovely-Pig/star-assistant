...
class MyBot(Wechaty):
    ...
    async def on_message(self, msg: Message):
        ...
        if not is_self:
            ...
            if room is None:
                ...
                # [开发者]-开发者的信息
                if from_contact in self.developers:
                    await self.devel_msg(userbot=userbot, msg=msg)
    ...
    async def say_hello(self, userbot: UserBot):
        ...
        file_box_intro = FileBox.from_file(path='static/introduction.mp4')
    ...
    async def process_img(self, msg: Message, filename: str, big_type: str, little_type: Optional[str] = None) -> Dict:
        """
        处理图片
        :param msg: Message
        :param filename: 图片的文件名
        :param big_type: 图像处理的模式，可选`alien`, `vegetable`, `environment`, `pet`
        :param little_type: 图像处理的模式，具体可参考`img_generate.py`

        """
        file_box = await msg.to_file_box()
        await file_box.to_file(file_path=filename, overwrite=True)
        res = self.imgs.run(img_path=filename, big_type=big_type, little_type=little_type)
        code, err, img, info = res['code'], res['err'], res['img'], res['info']
        print('res:', res)
    ...
    async def scan_img(self, userbot: UserBot, msg: Message):
        ...
        res = await self.process_img(msg=msg, filename=userbot.filename, big_type=big_type)
        if res['success']:
            info, file_box = res['info'], res['file_box']
    ...
    async def execute_task(self, task: Task, userbot: UserBot, msg: Message):
        ...
        if userbot.task:
            ...
            res = await self.process_img(msg=msg, filename=userbot.filename, big_type=task.big_type, little_type=task.little_type)
            if res['success']:
                info, file_box = res['info'], res['file_box']
                # 如果任务没有被其他人完成且没有超时
                if userbot.task:
                    ...
                    res = self.db.award(table=TABLE_USERS, user_name=conversation.name, score=task.score)
    ...
    async def execute_room_task(self, task: Task, room: Room, msg: Message):
        ...
        res = await self.process_img(msg=msg, filename=filename, big_type=task.big_type, little_type=task.little_type)
    ...
    async def devel_msg(self, userbot: UserBot, msg: Message):
        """
        处理开发者的信息
        """
        text = msg.text()
        ...
        if text == '发布任务':
            ...
            await self.how_to_write_task(conversation=conversation)
        ...
        if text == '注册用户':
            userbot.on_sign_up()
            await self.say_something(conversation=conversation, content=f'请按照如下示例指明用户')
            await self.say_something(conversation=conversation, content='【用户昵称】\n细菌')
        
        if text == '所有用户':
            user_names = '\n• '.join([_userbot.name for _userbot in self.userbots])
            await self.say_something(conversation=conversation, content=f'所有用户如下\n• {user_names}')                    
        ...
        if text == '发布群聊任务':
            ...
            await self.how_to_write_room_task(conversation=conversation)
        ...
        if text == '给所有用户发信息':
            userbot.say_to_all()
            await self.say_something(conversation=conversation, content='请编辑一条您要发送的信息📝\n文字或图片都可以')
